...
export interface UrlRulesApi {
     ...
     *
     * #### Example:
     * When no other url rule matches, redirect to `hostPage` and provide a `dashboard` parameter value.
     * ```js
     * .otherwise({ state: 'hostPage', params: { dashboard: 'default' } });
     * ```
     *
     ...
     *
     * #### Example:
     * When no other url rule matches, manually trigger a transition to the `hostPage` state
     * ```js
     * .otherwise((urlParts, router) => {
     *   router.stateService.go('hostPage');
     * });
     * ```
     *
     * #### Example:
     * When no other url rule matches, go to `hostPage` state
     * ```js
     * .otherwise((urlParts, router) => {
     *   return { state: 'hostPage' };
     * });
     * ```
     *
     ...
     *
     * #### Example:
     * Start app at `hostPage` state.
     * ```js
     * .initial({ state: 'hostPage' });
     * ```
     *
     * #### Example:
     * Start app at `/hostPage` (by url)
     * ```js
     * .initial('/hostPage');
     * ```
     *
